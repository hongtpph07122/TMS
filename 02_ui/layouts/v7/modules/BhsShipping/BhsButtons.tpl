{strip}
    {if $smarty.request.view eq 'List'}
      <div class="mx-auto" style="display:flex">
          <button type="button" class="btn module-buttons btn-bhs btnImportExcel"><i class="fa fa-upload" aria-hidden="true" style="color:white"></i><span class="hidden-sm hidden-xs">{vtranslate('Import', $MODULE)}</span></button>
        {if $GEO_LOCATION === 'TH'}
          <button type="button" class="btn module-buttons btn-bhs btnUpdateRescue"><i class="fa fa-retweet" aria-hidden="true" style="color:white"></i><span class="hidden-sm hidden-xs">{vtranslate('Update Rescue Jobs', $MODULE)}</span></button>
        {/if}
