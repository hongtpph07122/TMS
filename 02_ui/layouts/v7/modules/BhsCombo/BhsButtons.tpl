{strip}
    {if $smarty.request.view eq 'List'}
        <div class="mx-auto" style="display:flex">
            <button id="btnAddCustomer" type="button" class="btn module-buttons btn-bhs btnCreateProduct">
                <i class="material-icons">add</i>
                <span class="hidden-sm hidden-xs">{vtranslate('ADD_COMBO', $MODULE)}</span>
            </button>
{*            <button type="button" class="btn module-buttons btn-bhs btnExportExcel"><i class="fa fa-download" aria-hidden="true" style="color:white"></i><span class="hidden-sm hidden-xs">{vtranslate('EXPORT_CSV', $MODULE)}</span></button>*}
