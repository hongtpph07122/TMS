		...
		<div class="module-action-content clearfix {$MODULE}-module-action-content">
			<div class="col-lg-5 col-md-5 module-breadcrumb module-breadcrumb-{$smarty.request.view} transitionsAllHalfSecond">
				{if $MODULE|strpos:'Bhs' === false}
					...
					{/if}
				{else}
{*					<a title="{vtranslate('LBL_HOME', $MODULE)}" href='index.php'><h4 class="module-title pull-left text-uppercase"> {vtranslate('LBL_HOME', $MODULE)} </h4></a>*}
{*					<p class="current-filter-name filter-name pull-left ">&nbsp;&nbsp;<span class="fa fa-angle-right" aria-hidden="true"></span>&nbsp;&nbsp;</p>*}
{*					<a title="{vtranslate($MODULE, $MODULE)}" href='#'><h4 class="module-title pull-left"> {vtranslate('LBL_'|cat:$SELECTED_MENU_CATEGORY, $MODULE)} </h4></a>*}
{*					<p class="current-filter-name filter-name pull-left ">&nbsp;&nbsp;<span class="fa fa-angle-right" aria-hidden="true"></span>&nbsp;&nbsp;</p>*}
					{assign var=userTMS value=Vtiger_Session::get('agent_info')}
					{if !empty($userTMS) && $MODULE eq 'BhsOrders' && $userTMS->info->orgId eq $NORI_CUSTOMER_ORGID}
						<a title="{vtranslate('BhsOrdersNori',$MODULE )}" href='index.php?module={$MODULE}&view=List&app={$SELECTED_MENU_CATEGORY}'><h4 class="module-title pull-left"> {vtranslate('BhsOrdersNori',$MODULE )} </h4></a>
					{else}
					<a title="{vtranslate($MODULE, $MODULE)}" href='index.php?module={$MODULE}&view=List&app={$SELECTED_MENU_CATEGORY}'><h4 class="module-title pull-left"> {vtranslate($MODULE, $MODULE)} </h4></a>
					{/if}
					{if $smarty.request.view neq 'List'}
						<p class="current-filter-name filter-name pull-left ">&nbsp;&nbsp;<span class="fa fa-angle-right" aria-hidden="true"></span>&nbsp;&nbsp;</p>
						<a title="{vtranslate($MODULE, $MODULE)}" href='#'><h4 class="module-title pull-left"> {vtranslate('LBL_'|cat:$smarty.request.view, $MODULE)} </h4></a>
					{/if}
				{/if}
			</div>
			<div class="col-lg-7 col-md-7 pull-right" style="margin:  0px; padding: 0px;">
				<div id="appnav" class="pull-right">
					<ul class="nav navbar-nav">
						{if $MODULE|strpos:'Bhs' === false}
							...
							{/if}
						{else}
							{include file="BhsButtons.tpl"|vtemplate_path:$MODULE}
{*							{include file="BhsSearch.tpl"|vtemplate_path:$MODULE}*}
						{/if}
					</ul>
				</div>
			</div>
