...
Vtiger.Class("Vtiger_DashBoard_Js",{
	...
	registerEvents : function() {
		...
		$('#tblMailList0').DataTable( {
			"lengthChange": false,
			"dom": "t",
			"ajax": "index.php?module=BhsOrderProcessing&action=ActionAjax&mode=getList",
			"columns": [
				{ "data": "checkbox" },
				{ "data": "leadId" },
				{ "data": "name" },
		...
		$('#monitor-date-now').on('click', function(){
			updateDashboard('');
		});

		$('#monitor-date-yesterday').on('click', function(){
			var yesterdayStart = moment().subtract(1, 'days').startOf('day').format("DD/MM/YYYY HH:mm:ss");
			var yesterdayEnd = moment().subtract(1, 'days').endOf('day').format("DD/MM/YYYY HH:mm:ss");
			var yesterday = yesterdayStart + ' - ' + yesterdayEnd;
			updateDashboard(yesterday);
		});

		$('#monitor-date-week').on('click', function(){
			var weekStart = moment().startOf('isoWeek').format("DD/MM/YYYY HH:mm:ss");
			var now = moment().format("DD/MM/YYYY HH:mm:ss");
			var week = weekStart + ' - ' + now;
			updateDashboard(week);
		});

		$('#monitor-date-month').on('click', function(){
			var monthStart = moment().startOf('month').format("DD/MM/YYYY HH:mm:ss");
			var now = moment().format("DD/MM/YYYY HH:mm:ss");
			var month = monthStart + ' - ' + now;
			updateDashboard(month);
		});

		$('#monitor-custom-date').on('click', function(){
			$('#searchDate').val('');
			clearInterval(updateMonitorInterval);
			$('.dashboard-input-group.input-group').removeClass('hidden');
		});

		$('#searchDate').on('apply.daterangepicker', function (ev, picker) {
			updateMonitor();
			updateMonitorInterval = setInterval(updateMonitor, 10000);
		});
	}
});
...
function updateDashboard(period) {
	...
	clearInterval(updateMonitorInterval);
	updateMonitor();
	$('.dashboard-input-group.input-group').addClass('hidden');
	updateMonitorInterval = setInterval(updateMonitor, 10000);
}

$(document).ready(function () {
	//    Dashboard home
	$('.dashboard-input-group i').click(function(){
		$('.dashboard-input-group input[name="dates"]').click();
	});
	$('.dashboard-input-group input[name="dates"]').daterangepicker({
