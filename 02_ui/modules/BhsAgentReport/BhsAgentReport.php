<?php
class BhsAgentReport extends CRMEntity {
	var $db, $log; // Used in class functions of CRMEntity

	var $table_name = 'vtiger_bhsagentreport';
	var $table_index= 'bhsagentreportid';
	 ...
	 */
	var $customFieldTable = Array('vtiger_bhsagentreportcf', 'bhsagentreportid');

	/**
	 * Mandatory for Saving, Include tables related to this module.
	 */
	var $tab_name = Array('vtiger_crmentity', 'vtiger_bhsagentreport', 'vtiger_bhsagentreportcf');
	...
	var $tab_name_index = Array(
		...
		'vtiger_bhsagentreport' => 'bhsagentreportid',
		'vtiger_bhsagentreportcf'=>'bhsagentreportid');
	...
	var $list_fields = Array (
        ...
        'Name' => Array('bhsagentreport', 'bhsagentreport_name'),
		'CreatedTime' => Array('crmentity', 'createdtime'),
	);
	var $list_fields_name = Array (
		/* Format: Field Label => fieldname */
		'Name' => 'bhsagentreport_name',
	...
	var $def_basicsearch_col = 'bhsagentreport_name';
	...
	var $mandatory_fields = Array('bhsagentreport_name');

	var $default_order_by = 'bhsagentreport_name';
	...
	function vtlib_handler($moduleName, $eventType) {
		...
		if($eventType == 'module.postinstall') {
			...
			vtws_deleteWebserviceEntity('BhsAgentReport');
    ...
    public function addDefaultModuleTypeEntity(){
		global $adb;
        // Check entity module
        $rs=$adb->query("SELECT * FROM `vtiger_ws_entity` WHERE `name`='BhsAgentReport'");
        if($adb->num_rows($rs) == 0) {
            ...
            $adb->pquery("INSERT INTO `vtiger_ws_entity` (`id`, `name`, `handler_path`, `handler_class`, `ismodule`) VALUES (?, ?, ?, ?, ?);",
                array($entityId, 'BhsAgentReport', 'include/Webservices/VtigerModuleOperation.php', 'VtigerModuleOperation', '1'));
        }
    }

    public function addEntityIdentifier(){
        global $adb;
        $moduleName = 'BhsAgentReport';
        ...
        if($adb->num_rows($result) > 0){
            ...
            if($adb->num_rows($result2)==0){
                $adb->pquery("INSERT INTO vtiger_entityname(tabid, modulename, tablename, fieldname, entityidfield, entityidcolumn) VALUES(?,?,?,?,?,?)",
                    Array($tabid, $moduleName, $this->table_name, 'bhsagentreport_name', $this->table_index, $this->table_index));
            }else{
