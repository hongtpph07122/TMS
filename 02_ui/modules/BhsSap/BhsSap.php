<?php
class BhsSap extends CRMEntity {
	var $db, $log; // Used in class functions of CRMEntity

	var $table_name = 'vtiger_BhsDHL';
	var $table_index= 'BhsDHLid';
	 ...
	 */
	var $customFieldTable = Array('vtiger_BhsDHLcf', 'BhsDHLid');

	/**
	 * Mandatory for Saving, Include tables related to this module.
	 */
	var $tab_name = Array('vtiger_crmentity', 'vtiger_BhsDHL', 'vtiger_BhsDHLcf');
	...
	var $tab_name_index = Array(
		...
		'vtiger_BhsDHL' => 'BhsDHLid',
		'vtiger_BhsDHLcf'=>'BhsDHLid');
	...
	var $def_basicsearch_col = 'BhsDHL_name';
	...
	var $mandatory_fields = Array('BhsDHL_name');

	var $default_order_by = 'BhsDHL_name';
	...
	function vtlib_handler($moduleName, $eventType) {
		...
		if($eventType == 'module.postinstall') {
			...
			vtws_deleteWebserviceEntity('BhsSap');
    ...
    public function addDefaultModuleTypeEntity(){
		global $adb;
        // Check entity module
        $rs=$adb->query("SELECT * FROM `vtiger_ws_entity` WHERE `name`='BhsDHL'");
        if($adb->num_rows($rs) == 0) {
            ...
            $adb->pquery("INSERT INTO `vtiger_ws_entity` (`id`, `name`, `handler_path`, `handler_class`, `ismodule`) VALUES (?, ?, ?, ?, ?);",
                array($entityId, 'BhsSap', 'include/Webservices/VtigerModuleOperation.php', 'VtigerModuleOperation', '1'));
        }
    }
        ...
        if($adb->num_rows($result) > 0){
            ...
            if($adb->num_rows($result2)==0){
                $adb->pquery("INSERT INTO vtiger_entityname(tabid, modulename, tablename, fieldname, entityidfield, entityidcolumn) VALUES(?,?,?,?,?,?)",
                    Array($tabid, $moduleName, $this->table_name, 'BhsDHL_name', $this->table_index, $this->table_index));
            }else{
