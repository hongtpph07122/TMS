<?php
class BhsAgentAssignment extends CRMEntity {
	var $db, $log; // Used in class functions of CRMEntity

	var $table_name = 'vtiger_bhsagentassignment';
	var $table_index= 'bhsagentassignmentid';
	 ...
	 */
	var $customFieldTable = Array('vtiger_bhsagentassignmentcf', 'bhsagentassignmentid');

	/**
	 * Mandatory for Saving, Include tables related to this module.
	 */
	var $tab_name = Array('vtiger_crmentity', 'vtiger_bhsagentassignment', 'vtiger_bhsagentassignmentcf');
	...
	var $tab_name_index = Array(
		...
		'vtiger_bhsagentassignment' => 'bhsagentassignmentid',
		'vtiger_bhsagentassignmentcf'=>'bhsagentassignmentid');
	...
	var $list_fields = Array (
        ...
        'Name' => Array('bhsagentassignment', 'bhsagentassignment_name'),
		'CreatedTime' => Array('crmentity', 'createdtime'),
	);
	var $list_fields_name = Array (
		/* Format: Field Label => fieldname */
		'Name' => 'bhsagentassignment_name',
	...
	var $def_basicsearch_col = 'bhsagentassignment_name';
	...
	var $mandatory_fields = Array('bhsagentassignment_name');

	var $default_order_by = 'bhsagentassignment_name';
	...
	function vtlib_handler($moduleName, $eventType) {
		...
		if($eventType == 'module.postinstall') {
			...
			vtws_deleteWebserviceEntity('BhsAgentAssignment');
    ...
    public function addModuleIcon(){
        ...
        $source1 = $root_directory . '/modules/BhsAgentAssignment/resources/BhsAgentAssignment.png';
        $source2 = $root_directory . '/modules/BhsAgentAssignment/resources/moduleImages/BhsAgentAssignment.png';
        $dest1 = $root_directory . '/layouts/v7/skins/images/BhsAgentAssignment.png';
        copy($source1, $dest1);
        $dest2 = $root_directory . '/layouts/v7/skins/images/moduleImages/BhsAgentAssignment.png';
        copy($source2, $dest2);
    }
    ...
    public function addDefaultModuleTypeEntity(){
		global $adb;
        // Check entity module
        $rs=$adb->query("SELECT * FROM `vtiger_ws_entity` WHERE `name`='BhsAgentAssignment'");
        if($adb->num_rows($rs) == 0) {
            ...
            $adb->pquery("INSERT INTO `vtiger_ws_entity` (`id`, `name`, `handler_path`, `handler_class`, `ismodule`) VALUES (?, ?, ?, ?, ?);",
                array($entityId, 'BhsAgentAssignment', 'include/Webservices/VtigerModuleOperation.php', 'VtigerModuleOperation', '1'));
        }

        // Add to menu
        $tabid = getTabid("BhsAgentAssignment");
        $appname = 'SALEORDER';
        $sql="SELECT * FROM vtiger_app2tab WHERE appname=? AND tabid=?";
        $res=$adb->pquery($sql, array($appname, $tabid));
        if($adb->num_rows($res) == 0) {
            $adb->pquery("INSERT INTO `vtiger_app2tab` (tabid, appname, sequence, visible) VALUES (?, ?, ?, ?)",
                array($tabid, $appname, 100, 1));
        }
    }

    public function addEntityIdentifier(){
        global $adb;
        $moduleName = 'BhsAgentAssignment';
        ...
        if($adb->num_rows($result) > 0){
            ...
            if($adb->num_rows($result2)==0){
                $adb->pquery("INSERT INTO vtiger_entityname(tabid, modulename, tablename, fieldname, entityidfield, entityidcolumn) VALUES(?,?,?,?,?,?)",
                    Array($tabid, $moduleName, $this->table_name, 'bhsagentassignment_name', $this->table_index, $this->table_index));
            }else{
