<?php
class BhsProduct extends CRMEntity {
	var $db, $log; // Used in class functions of CRMEntity

	var $table_name = 'vtiger_bhsproduct';
	var $table_index= 'bhsproductid';
	 ...
	 */
	var $customFieldTable = Array('vtiger_bhsproductcf', 'bhsproductid');

	/**
	 * Mandatory for Saving, Include tables related to this module.
	 */
	var $tab_name = Array('vtiger_crmentity', 'vtiger_bhsproduct', 'vtiger_bhsproductcf');
	...
	var $tab_name_index = Array(
		...
		'vtiger_bhsproduct' => 'bhsproductid',
		'vtiger_bhsproductcf'=>'bhsproductid');
	...
	var $list_fields = Array (
        ...
        'Name' => Array('bhsproduct', 'bhsproduct_name'),
		'CreatedTime' => Array('crmentity', 'createdtime'),
	);
	var $list_fields_name = Array (
		/* Format: Field Label => fieldname */
		'Name' => 'bhsproduct_name',
	...
	var $def_basicsearch_col = 'bhsproduct_name';
	...
	var $mandatory_fields = Array('bhsproduct_name');

	var $default_order_by = 'bhsproduct_name';
	...
	function vtlib_handler($moduleName, $eventType) {
		...
		if($eventType == 'module.postinstall') {
			...
			vtws_deleteWebserviceEntity('BhsProduct');
    ...
    public function addDefaultModuleTypeEntity(){
		global $adb;
        // Check entity module
        $rs=$adb->query("SELECT * FROM `vtiger_ws_entity` WHERE `name`='BhsProduct'");
        if($adb->num_rows($rs) == 0) {
            ...
            $adb->pquery("INSERT INTO `vtiger_ws_entity` (`id`, `name`, `handler_path`, `handler_class`, `ismodule`) VALUES (?, ?, ?, ?, ?);",
                array($entityId, 'BhsProduct', 'include/Webservices/VtigerModuleOperation.php', 'VtigerModuleOperation', '1'));
        }
    }

    public function addEntityIdentifier(){
        global $adb;
        $moduleName = 'BhsProduct';
        ...
        if($adb->num_rows($result) > 0){
            ...
            if($adb->num_rows($result2)==0){
                $adb->pquery("INSERT INTO vtiger_entityname(tabid, modulename, tablename, fieldname, entityidfield, entityidcolumn) VALUES(?,?,?,?,?,?)",
                    Array($tabid, $moduleName, $this->table_name, 'bhsproduct_name', $this->table_index, $this->table_index));
            }else{
