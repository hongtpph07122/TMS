<?php
class BhsCampaignManagement extends CRMEntity {
	var $db, $log; // Used in class functions of CRMEntity

	var $table_name = 'vtiger_bhscampaignmanagement';
	var $table_index= 'bhscampaignmanagementid';
	 ...
	 */
	var $customFieldTable = Array('vtiger_bhscampaignmanagementcf', 'bhscampaignmanagementid');

	/**
	 * Mandatory for Saving, Include tables related to this module.
	 */
	var $tab_name = Array('vtiger_crmentity', 'vtiger_bhscampaignmanagement', 'vtiger_bhscampaignmanagementcf');
	...
	var $tab_name_index = Array(
		...
		'vtiger_bhscampaignmanagement' => 'bhscampaignmanagementid',
		'vtiger_bhscampaignmanagementcf'=>'bhscampaignmanagementid');
	...
	var $list_fields = Array (
        ...
        'Name' => Array('bhscampaignmanagement', 'bhscampaignmanagement_name'),
		'CreatedTime' => Array('crmentity', 'createdtime'),
	);
	var $list_fields_name = Array (
		/* Format: Field Label => fieldname */
		'Name' => 'bhscampaignmanagement_name',
	...
	var $def_basicsearch_col = 'bhscampaignmanagement_name';
	...
	var $mandatory_fields = Array('bhscampaignmanagement_name');

	var $default_order_by = 'bhscampaignmanagement_name';
	...
	function vtlib_handler($moduleName, $eventType) {
		...
		if($eventType == 'module.postinstall') {
			...
			vtws_deleteWebserviceEntity('BhsCampaignManagement');
    ...
    public function addDefaultModuleTypeEntity(){
		global $adb;
        // Check entity module
        $rs=$adb->query("SELECT * FROM `vtiger_ws_entity` WHERE `name`='BhsCampaignManagement'");
        if($adb->num_rows($rs) == 0) {
            ...
            $adb->pquery("INSERT INTO `vtiger_ws_entity` (`id`, `name`, `handler_path`, `handler_class`, `ismodule`) VALUES (?, ?, ?, ?, ?);",
                array($entityId, 'BhsCampaignManagement', 'include/Webservices/VtigerModuleOperation.php', 'VtigerModuleOperation', '1'));
        }
    }

    public function addEntityIdentifier(){
        global $adb;
        $moduleName = 'BhsCampaignManagement';
        ...
        if($adb->num_rows($result) > 0){
            ...
            if($adb->num_rows($result2)==0){
                $adb->pquery("INSERT INTO vtiger_entityname(tabid, modulename, tablename, fieldname, entityidfield, entityidcolumn) VALUES(?,?,?,?,?,?)",
                    Array($tabid, $moduleName, $this->table_name, 'bhscampaignmanagement_name', $this->table_index, $this->table_index));
            }else{
