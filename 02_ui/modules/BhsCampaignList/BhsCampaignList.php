<?php
class BhsCampaignList extends CRMEntity {
	var $db, $log; // Used in class functions of CRMEntity

	var $table_name = 'vtiger_bhscampaignlist';
	var $table_index= 'bhscampaignlistid';
	 ...
	 */
	var $customFieldTable = Array('vtiger_bhscampaignlistcf', 'bhscampaignlistid');

	/**
	 * Mandatory for Saving, Include tables related to this module.
	 */
	var $tab_name = Array('vtiger_crmentity', 'vtiger_bhscampaignlist', 'vtiger_bhscampaignlistcf');
	...
	var $tab_name_index = Array(
		...
		'vtiger_bhscampaignlist' => 'bhscampaignlistid',
		'vtiger_bhscampaignlistcf'=>'bhscampaignlistid');
	...
	var $list_fields = Array (
        ...
        'Name' => Array('bhscampaignlist', 'bhscampaignlist_name'),
		'CreatedTime' => Array('crmentity', 'createdtime'),
	);
	var $list_fields_name = Array (
		/* Format: Field Label => fieldname */
		'Name' => 'bhscampaignlist_name',
	...
	var $def_basicsearch_col = 'bhscampaignlist_name';
	...
	var $mandatory_fields = Array('bhscampaignlist_name');

	var $default_order_by = 'bhscampaignlist_name';
	...
	function vtlib_handler($moduleName, $eventType) {
		...
		if($eventType == 'module.postinstall') {
			...
			vtws_deleteWebserviceEntity('BhsCampaignList');
    ...
    public function addDefaultModuleTypeEntity(){
		global $adb;
        // Check entity module
        $rs=$adb->query("SELECT * FROM `vtiger_ws_entity` WHERE `name`='BhsCampaignList'");
        if($adb->num_rows($rs) == 0) {
            ...
            $adb->pquery("INSERT INTO `vtiger_ws_entity` (`id`, `name`, `handler_path`, `handler_class`, `ismodule`) VALUES (?, ?, ?, ?, ?);",
                array($entityId, 'BhsCampaignList', 'include/Webservices/VtigerModuleOperation.php', 'VtigerModuleOperation', '1'));
        }
    }

    public function addEntityIdentifier(){
        global $adb;
        $moduleName = 'BhsCampaignList';
        ...
        if($adb->num_rows($result) > 0){
            ...
            if($adb->num_rows($result2)==0){
                $adb->pquery("INSERT INTO vtiger_entityname(tabid, modulename, tablename, fieldname, entityidfield, entityidcolumn) VALUES(?,?,?,?,?,?)",
                    Array($tabid, $moduleName, $this->table_name, 'bhscampaignlist_name', $this->table_index, $this->table_index));
            }else{
