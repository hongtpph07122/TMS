<?php
class BhsCdrs extends CRMEntity {
	var $db, $log; // Used in class functions of CRMEntity

	var $table_name = 'vtiger_bhscdrs';
	var $table_index= 'bhscdrsid';
	 ...
	 */
	var $customFieldTable = Array('vtiger_bhscdrscf', 'bhscdrsid');

	/**
	 * Mandatory for Saving, Include tables related to this module.
	 */
	var $tab_name = Array('vtiger_crmentity', 'vtiger_bhscdrs', 'vtiger_bhscdrscf');
	...
	var $tab_name_index = Array(
		...
		'vtiger_bhscdrs' => 'bhscdrsid',
		'vtiger_bhscdrscf'=>'bhscdrsid');
	...
	var $list_fields = Array (
        ...
        'Name' => Array('bhscdrs', 'bhscdrs_name'),
	...
	var $def_basicsearch_col = 'bhscdrs_name';
	...
	var $mandatory_fields = Array('bhscdrs_name');

	var $default_order_by = 'bhscdrs_name';
	...
	function vtlib_handler($moduleName, $eventType) {
		...
		if($eventType == 'module.postinstall') {
			...
			vtws_deleteWebserviceEntity('BhsCdrs');
    ...
    public function addDefaultModuleTypeEntity(){
		global $adb;
        // Check entity module
        $rs=$adb->query("SELECT * FROM `vtiger_ws_entity` WHERE `name`='BhsCdrs'");
        if($adb->num_rows($rs) == 0) {
            ...
            $adb->pquery("INSERT INTO `vtiger_ws_entity` (`id`, `name`, `handler_path`, `handler_class`, `ismodule`) VALUES (?, ?, ?, ?, ?);",
                array($entityId, 'BhsCdrs', 'include/Webservices/VtigerModuleOperation.php', 'VtigerModuleOperation', '1'));
        }
    }
        ...
        if($adb->num_rows($result) > 0){
            ...
            if($adb->num_rows($result2)==0){
                $adb->pquery("INSERT INTO vtiger_entityname(tabid, modulename, tablename, fieldname, entityidfield, entityidcolumn) VALUES(?,?,?,?,?,?)",
                    Array($tabid, $moduleName, $this->table_name, 'bhscdrs_name', $this->table_index, $this->table_index));
            }else{
