<?php
class BhsCampaign extends CRMEntity {
	var $db, $log; // Used in class functions of CRMEntity

	var $table_name = 'vtiger_bhscampaign';
	var $table_index= 'bhscampaignid';
	 ...
	 */
	var $customFieldTable = Array('vtiger_bhscampaigncf', 'bhscampaignid');

	/**
	 * Mandatory for Saving, Include tables related to this module.
	 */
	var $tab_name = Array('vtiger_crmentity', 'vtiger_bhscampaign', 'vtiger_bhscampaigncf');
	...
	var $tab_name_index = Array(
		...
		'vtiger_bhscampaign' => 'bhscampaignid',
		'vtiger_bhscampaigncf'=>'bhscampaignid');
	...
	var $list_fields = Array (
        ...
        'Name' => Array('bhscampaign', 'campaign_name'),
	...
	var $mandatory_fields = Array('campaign_name');
	...
	function vtlib_handler($moduleName, $eventType) {
		...
		if($eventType == 'module.postinstall') {
			...
			vtws_deleteWebserviceEntity('BhsCampaign');
    ...
    public function addModuleIcon(){
        ...
        $source1 = $root_directory . '/modules/BhsCampaign/resources/BhsCampaign.png';
        $source2 = $root_directory . '/modules/BhsCampaign/resources/moduleImages/BhsCampaign.png';
        $dest1 = $root_directory . '/layouts/v7/skins/images/BhsCampaign.png';
        copy($source1, $dest1);
        $dest2 = $root_directory . '/layouts/v7/skins/images/moduleImages/BhsCampaign.png';
        copy($source2, $dest2);
    }

    private function addWidgetTo() {
        $widgetType = 'DASHBOARDWIDGET';
        $widgetName = 'Campaigns';
        $widgetUrl = 'index.php?module=BhsCampaign&view=ShowWidget&name=CampaignsByStatus';
    ...
    public function addDefaultModuleTypeEntity(){
		global $adb;
        // Check entity module
        $rs=$adb->query("SELECT * FROM `vtiger_ws_entity` WHERE `name`='BhsCampaign'");
        if($adb->num_rows($rs) == 0) {
            ...
            $adb->pquery("INSERT INTO `vtiger_ws_entity` (`id`, `name`, `handler_path`, `handler_class`, `ismodule`) VALUES (?, ?, ?, ?, ?);",
                array($entityId, 'BhsCampaign', 'include/Webservices/VtigerModuleOperation.php', 'VtigerModuleOperation', '1'));
        }

        // Add to menu
        $tabid = getTabid("BhsCampaign");
        $appname = 'CAMPAIGN';
        ...
        if($adb->num_rows($result) > 0){
            ...
            if($adb->num_rows($result2)==0){
                $adb->pquery("INSERT INTO vtiger_entityname(tabid, modulename, tablename, fieldname, entityidfield, entityidcolumn) VALUES(?,?,?,?,?,?)",
                    Array($tabid, $moduleName, $this->table_name, 'campaign_name', $this->table_index, $this->table_index));
            }else{
