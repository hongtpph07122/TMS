<?php
class BhsCallbackManagement extends CRMEntity {
	var $db, $log; // Used in class functions of CRMEntity

	var $table_name = 'vtiger_bhscallbackmanagement';
	var $table_index= 'bhscallbackmanagementid';
	 ...
	 */
	var $customFieldTable = Array('vtiger_bhscallbackmanagementcf', 'bhscallbackmanagementid');

	/**
	 * Mandatory for Saving, Include tables related to this module.
	 */
	var $tab_name = Array('vtiger_crmentity', 'vtiger_bhscallbackmanagement', 'vtiger_bhscallbackmanagementcf');
	...
	var $tab_name_index = Array(
		...
		'vtiger_bhscallbackmanagement' => 'bhscallbackmanagementid',
		'vtiger_bhscallbackmanagementcf'=>'bhscallbackmanagementid');
	...
	var $list_fields = Array (
        ...
        'Name' => Array('bhscallbackmanagement', 'bhscallbackmanagement_name'),
		'CreatedTime' => Array('crmentity', 'createdtime'),
	);
	var $list_fields_name = Array (
		/* Format: Field Label => fieldname */
		'Name' => 'bhscallbackmanagement_name',
	...
	var $def_basicsearch_col = 'bhscallbackmanagement_name';
	...
	var $mandatory_fields = Array('bhscallbackmanagement_name');

	var $default_order_by = 'bhscallbackmanagement_name';
	...
	function vtlib_handler($moduleName, $eventType) {
		...
		if($eventType == 'module.postinstall') {
			...
			vtws_deleteWebserviceEntity('BhsCallbackManagement');
    ...
    private function addWidgetTo() {
        ...
        $widgetName = 'Calls By Status';
        $widgetUrl = 'index.php?module=BhsCallbackManagement&view=ShowWidget&name=CallsByStatus';
    ...
    public function addDefaultModuleTypeEntity(){
		global $adb;
        // Check entity module
        $rs=$adb->query("SELECT * FROM `vtiger_ws_entity` WHERE `name`='BhsCallbackManagement'");
        if($adb->num_rows($rs) == 0) {
            ...
            $adb->pquery("INSERT INTO `vtiger_ws_entity` (`id`, `name`, `handler_path`, `handler_class`, `ismodule`) VALUES (?, ?, ?, ?, ?);",
                array($entityId, 'BhsCallbackManagement', 'include/Webservices/VtigerModuleOperation.php', 'VtigerModuleOperation', '1'));
        }
    }

    public function addEntityIdentifier(){
        global $adb;
        $moduleName = 'BhsCallbackManagement';
        ...
        if($adb->num_rows($result) > 0){
            ...
            if($adb->num_rows($result2)==0){
                $adb->pquery("INSERT INTO vtiger_entityname(tabid, modulename, tablename, fieldname, entityidfield, entityidcolumn) VALUES(?,?,?,?,?,?)",
                    Array($tabid, $moduleName, $this->table_name, 'bhscallbackmanagement_name', $this->table_index, $this->table_index));
            }else{
