<?php
class BhsBlacklist extends CRMEntity {
	var $db, $log; // Used in class functions of CRMEntity

	var $table_name = 'vtiger_bhsblacklist';
	var $table_index= 'bhsblacklistid';
	 ...
	 */
	var $customFieldTable = Array('vtiger_bhsblacklistcf', 'bhsblacklistid');

	/**
	 * Mandatory for Saving, Include tables related to this module.
	 */
	var $tab_name = Array('vtiger_crmentity', 'vtiger_bhsblacklist', 'vtiger_bhsblacklistcf');
	...
	var $tab_name_index = Array(
		...
		'vtiger_bhsblacklist' => 'bhsblacklistid',
		'vtiger_bhsblacklistcf'=>'bhsblacklistid');
	...
	var $list_fields = Array (
        ...
        'Name' => Array('bhsblacklist', 'bhsblacklist_name'),
		'CreatedTime' => Array('crmentity', 'createdtime'),
	);
	var $list_fields_name = Array (
		/* Format: Field Label => fieldname */
		'Name' => 'bhsblacklist_name',
	...
	var $def_basicsearch_col = 'bhsblacklist_name';
	...
	var $mandatory_fields = Array('bhsblacklist_name');

	var $default_order_by = 'bhsblacklist_name';
	...
	function vtlib_handler($moduleName, $eventType) {
		...
		if($eventType == 'module.postinstall') {
			...
			vtws_deleteWebserviceEntity('BhsBlacklist');
    ...
    public function addDefaultModuleTypeEntity(){
		global $adb;
        // Check entity module
        $rs=$adb->query("SELECT * FROM `vtiger_ws_entity` WHERE `name`='BhsBlacklist'");
        if($adb->num_rows($rs) == 0) {
            ...
            $adb->pquery("INSERT INTO `vtiger_ws_entity` (`id`, `name`, `handler_path`, `handler_class`, `ismodule`) VALUES (?, ?, ?, ?, ?);",
                array($entityId, 'BhsBlacklist', 'include/Webservices/VtigerModuleOperation.php', 'VtigerModuleOperation', '1'));
        }
    }

    public function addEntityIdentifier(){
        global $adb;
        $moduleName = 'BhsBlacklist';
        ...
        if($adb->num_rows($result) > 0){
            ...
            if($adb->num_rows($result2)==0){
                $adb->pquery("INSERT INTO vtiger_entityname(tabid, modulename, tablename, fieldname, entityidfield, entityidcolumn) VALUES(?,?,?,?,?,?)",
                    Array($tabid, $moduleName, $this->table_name, 'bhsblacklist_name', $this->table_index, $this->table_index));
            }else{
