 ...
 * All Rights Reserved.
 * ****************************************************************************** */
class BhsDnc_ActionAjax_Action extends Vtiger_BhsBasicAjax_Action {
    ...
    function getList(Vtiger_Request $request){
        $moduleModel = new BhsDnc_Module_Model();
        $draw = intval($request->get('draw'));
        try{
            $data = array();
            $rows = $moduleModel->getRows($request, 'dnc_list');
            foreach ($rows as $row){
                $id = $row['lead_id'];
                $item = $row;
                $item['bhs_action'] = '<button type="button" data-id="'. $id .'" class="btn btn-lg btn-toggle" data-toggle="button" aria-pressed="false" autocomplete="off">
