<?php
class BhsDnc extends CRMEntity {
	var $db, $log; // Used in class functions of CRMEntity

	var $table_name = 'vtiger_bhsdnc';
	var $table_index= 'bhsdncid';
	 ...
	 */
	var $customFieldTable = Array('vtiger_bhsdnccf', 'bhsdncid');

	/**
	 * Mandatory for Saving, Include tables related to this module.
	 */
	var $tab_name = Array('vtiger_crmentity', 'vtiger_bhsdnc', 'vtiger_bhsdnccf');
	...
	var $tab_name_index = Array(
		...
		'vtiger_bhsdnc' => 'bhsdncid',
		'vtiger_bhsdnccf'=>'bhsdncid');
	...
	var $def_basicsearch_col = 'bhsdnc_name';
	...
	var $mandatory_fields = Array('bhsdnc_name');

	var $default_order_by = 'bhsdnc_name';
	...
	function vtlib_handler($moduleName, $eventType) {
		...
		if($eventType == 'module.postinstall') {
			...
			vtws_deleteWebserviceEntity('BhsDnc');
    ...
    public function addDefaultModuleTypeEntity(){
		global $adb;
        // Check entity module
        $rs=$adb->query("SELECT * FROM `vtiger_ws_entity` WHERE `name`='BhsDnc'");
        if($adb->num_rows($rs) == 0) {
            ...
            $adb->pquery("INSERT INTO `vtiger_ws_entity` (`id`, `name`, `handler_path`, `handler_class`, `ismodule`) VALUES (?, ?, ?, ?, ?);",
                array($entityId, 'BhsDnc', 'include/Webservices/VtigerModuleOperation.php', 'VtigerModuleOperation', '1'));
        }
    }
        ...
        if($adb->num_rows($result) > 0){
            ...
            if($adb->num_rows($result2)==0){
                $adb->pquery("INSERT INTO vtiger_entityname(tabid, modulename, tablename, fieldname, entityidfield, entityidcolumn) VALUES(?,?,?,?,?,?)",
                    Array($tabid, $moduleName, $this->table_name, 'bhsdnc_name', $this->table_index, $this->table_index));
            }else{
