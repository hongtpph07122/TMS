<?php
class BhsMissedCalls extends CRMEntity {
	var $db, $log; // Used in class functions of CRMEntity

	var $table_name = 'vtiger_bhsmissedcalls';
	var $table_index= 'bhsmissedcallsid';
	 ...
	 */
	var $customFieldTable = Array('vtiger_bhsmissedcallscf', 'bhsmissedcallsid');

	/**
	 * Mandatory for Saving, Include tables related to this module.
	 */
	var $tab_name = Array('vtiger_crmentity', 'vtiger_bhsmissedcalls', 'vtiger_bhsmissedcallscf');
	...
	var $tab_name_index = Array(
		...
		'vtiger_bhsmissedcalls' => 'bhsmissedcallsid',
		'vtiger_bhsmissedcallscf'=>'bhsmissedcallsid');
	...
	var $list_fields = Array (
        ...
        'Name' => Array('bhsmissedcalls', 'bhsmissedcalls_name'),
		'CreatedTime' => Array('crmentity', 'createdtime'),
	);
	var $list_fields_name = Array (
		/* Format: Field Label => fieldname */
		'Name' => 'bhsmissedcalls_name',
	...
	var $def_basicsearch_col = 'bhsmissedcalls_name';
	...
	var $mandatory_fields = Array('bhsmissedcalls_name');

	var $default_order_by = 'bhsmissedcalls_name';
	...
	function vtlib_handler($moduleName, $eventType) {
		...
		if($eventType == 'module.postinstall') {
			...
			vtws_deleteWebserviceEntity('BhsMissedCalls');
    ...
    public function addDefaultModuleTypeEntity(){
		global $adb;
        // Check entity module
        $rs=$adb->query("SELECT * FROM `vtiger_ws_entity` WHERE `name`='BhsMissedCalls'");
        if($adb->num_rows($rs) == 0) {
            ...
            $adb->pquery("INSERT INTO `vtiger_ws_entity` (`id`, `name`, `handler_path`, `handler_class`, `ismodule`) VALUES (?, ?, ?, ?, ?);",
                array($entityId, 'BhsMissedCalls', 'include/Webservices/VtigerModuleOperation.php', 'VtigerModuleOperation', '1'));
        }
    }

    public function addEntityIdentifier(){
        global $adb;
        $moduleName = 'BhsMissedCalls';
        ...
        if($adb->num_rows($result) > 0){
            ...
            if($adb->num_rows($result2)==0){
                $adb->pquery("INSERT INTO vtiger_entityname(tabid, modulename, tablename, fieldname, entityidfield, entityidcolumn) VALUES(?,?,?,?,?,?)",
                    Array($tabid, $moduleName, $this->table_name, 'bhsmissedcalls_name', $this->table_index, $this->table_index));
            }else{
