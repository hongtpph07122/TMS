<?php
class BhsCampaignReport extends CRMEntity {
	var $db, $log; // Used in class functions of CRMEntity

	var $table_name = 'vtiger_bhscampaignreport';
	var $table_index= 'bhscampaignreportid';
	 ...
	 */
	var $customFieldTable = Array('vtiger_bhscampaignreportcf', 'bhscampaignreportid');

	/**
	 * Mandatory for Saving, Include tables related to this module.
	 */
	var $tab_name = Array('vtiger_crmentity', 'vtiger_bhscampaignreport', 'vtiger_bhscampaignreportcf');
	...
	var $tab_name_index = Array(
		...
		'vtiger_bhscampaignreport' => 'bhscampaignreportid',
		'vtiger_bhscampaignreportcf'=>'bhscampaignreportid');
	...
	var $list_fields = Array (
        ...
        'Name' => Array('bhscampaignreport', 'bhscampaignreport_name'),
		'CreatedTime' => Array('crmentity', 'createdtime'),
	);
	var $list_fields_name = Array (
		/* Format: Field Label => fieldname */
		'Name' => 'bhscampaignreport_name',
	...
	var $def_basicsearch_col = 'bhscampaignreport_name';
	...
	var $mandatory_fields = Array('bhscampaignreport_name');

	var $default_order_by = 'bhscampaignreport_name';
	...
	function vtlib_handler($moduleName, $eventType) {
		...
		if($eventType == 'module.postinstall') {
			...
			vtws_deleteWebserviceEntity('BhsCampaignReport');
    ...
    public function addDefaultModuleTypeEntity(){
		global $adb;
        // Check entity module
        $rs=$adb->query("SELECT * FROM `vtiger_ws_entity` WHERE `name`='BhsCampaignReport'");
        if($adb->num_rows($rs) == 0) {
            ...
            $adb->pquery("INSERT INTO `vtiger_ws_entity` (`id`, `name`, `handler_path`, `handler_class`, `ismodule`) VALUES (?, ?, ?, ?, ?);",
                array($entityId, 'BhsCampaignReport', 'include/Webservices/VtigerModuleOperation.php', 'VtigerModuleOperation', '1'));
        }
    }

    public function addEntityIdentifier(){
        global $adb;
        $moduleName = 'BhsCampaignReport';
        ...
        if($adb->num_rows($result) > 0){
            ...
            if($adb->num_rows($result2)==0){
                $adb->pquery("INSERT INTO vtiger_entityname(tabid, modulename, tablename, fieldname, entityidfield, entityidcolumn) VALUES(?,?,?,?,?,?)",
                    Array($tabid, $moduleName, $this->table_name, 'bhscampaignreport_name', $this->table_index, $this->table_index));
            }else{
