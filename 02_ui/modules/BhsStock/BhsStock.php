<?php
class BhsStock extends CRMEntity {
	var $db, $log; // Used in class functions of CRMEntity

	var $table_name = 'vtiger_bhsstock';
	var $table_index= 'bhsstockid';
	 ...
	 */
	var $customFieldTable = Array('vtiger_bhsstockcf', 'bhsstockid');

	/**
	 * Mandatory for Saving, Include tables related to this module.
	 */
	var $tab_name = Array('vtiger_crmentity', 'vtiger_bhsstock', 'vtiger_bhsstockcf');
	...
	var $tab_name_index = Array(
		...
		'vtiger_bhsstock' => 'bhsstockid',
		'vtiger_bhsstockcf'=>'bhsstockid');
	...
	var $list_fields = Array (
        ...
        'Name' => Array('bhsstock', 'bhsstock_name'),
	...
	var $def_basicsearch_col = 'bhsstock_name';
	...
	var $mandatory_fields = Array('bhsstock_name');

	var $default_order_by = 'bhsstock_name';
	...
	function vtlib_handler($moduleName, $eventType) {
		...
		if($eventType == 'module.postinstall') {
			...
			vtws_deleteWebserviceEntity('BhsStock');
    ...
    public function addDefaultModuleTypeEntity(){
		global $adb;
        // Check entity module
        $rs=$adb->query("SELECT * FROM `vtiger_ws_entity` WHERE `name`='BhsStock'");
        if($adb->num_rows($rs) == 0) {
            ...
            $adb->pquery("INSERT INTO `vtiger_ws_entity` (`id`, `name`, `handler_path`, `handler_class`, `ismodule`) VALUES (?, ?, ?, ?, ?);",
                array($entityId, 'BhsStock', 'include/Webservices/VtigerModuleOperation.php', 'VtigerModuleOperation', '1'));
        }
    }
        ...
        if($adb->num_rows($result) > 0){
            ...
            if($adb->num_rows($result2)==0){
                $adb->pquery("INSERT INTO vtiger_entityname(tabid, modulename, tablename, fieldname, entityidfield, entityidcolumn) VALUES(?,?,?,?,?,?)",
                    Array($tabid, $moduleName, $this->table_name, 'bhsstock_name', $this->table_index, $this->table_index));
            }else{
