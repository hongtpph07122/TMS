<?php
class BhsShipping extends CRMEntity {
	var $db, $log; // Used in class functions of CRMEntity

	var $table_name = 'vtiger_bhsshipping';
	var $table_index= 'bhsshippingid';
	 ...
	 */
	var $customFieldTable = Array('vtiger_bhsshippingcf', 'bhsshippingid');

	/**
	 * Mandatory for Saving, Include tables related to this module.
	 */
	var $tab_name = Array('vtiger_crmentity', 'vtiger_bhsshipping', 'vtiger_bhsshippingcf');
	...
	var $tab_name_index = Array(
		...
		'vtiger_bhsshipping' => 'bhsshippingid',
		'vtiger_bhsshippingcf'=>'bhsshippingid');
	...
	var $list_fields = Array (
        ...
        'Name' => Array('bhsshipping', 'bhsshipping_name'),
		'CreatedTime' => Array('crmentity', 'createdtime'),
	);
	var $list_fields_name = Array (
		/* Format: Field Label => fieldname */
		'Name' => 'bhsshipping_name',
	...
	var $def_basicsearch_col = 'bhsshipping_name';
	...
	var $mandatory_fields = Array('bhsshipping_name');

	var $default_order_by = 'bhsshipping_name';
	...
	function vtlib_handler($moduleName, $eventType) {
		...
		if($eventType == 'module.postinstall') {
			...
			vtws_deleteWebserviceEntity('BhsShipping');
    ...
    public function addDefaultModuleTypeEntity(){
		global $adb;
        // Check entity module
        $rs=$adb->query("SELECT * FROM `vtiger_ws_entity` WHERE `name`='BhsShipping'");
        if($adb->num_rows($rs) == 0) {
            ...
            $adb->pquery("INSERT INTO `vtiger_ws_entity` (`id`, `name`, `handler_path`, `handler_class`, `ismodule`) VALUES (?, ?, ?, ?, ?);",
                array($entityId, 'BhsShipping', 'include/Webservices/VtigerModuleOperation.php', 'VtigerModuleOperation', '1'));
        }
    }

    public function addEntityIdentifier(){
        global $adb;
        $moduleName = 'BhsShipping';
        ...
        if($adb->num_rows($result) > 0){
            ...
            if($adb->num_rows($result2)==0){
                $adb->pquery("INSERT INTO vtiger_entityname(tabid, modulename, tablename, fieldname, entityidfield, entityidcolumn) VALUES(?,?,?,?,?,?)",
                    Array($tabid, $moduleName, $this->table_name, 'bhsshipping_name', $this->table_index, $this->table_index));
            }else{
