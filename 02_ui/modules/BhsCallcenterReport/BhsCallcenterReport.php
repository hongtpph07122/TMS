<?php
class BhsCallcenterReport extends CRMEntity {
	var $db, $log; // Used in class functions of CRMEntity

	var $table_name = 'vtiger_bhscallcenterreport';
	var $table_index= 'bhscallcenterreportid';
	 ...
	 */
	var $customFieldTable = Array('vtiger_bhscallcenterreportcf', 'bhscallcenterreportid');

	/**
	 * Mandatory for Saving, Include tables related to this module.
	 */
	var $tab_name = Array('vtiger_crmentity', 'vtiger_bhscallcenterreport', 'vtiger_bhscallcenterreportcf');
	...
	var $tab_name_index = Array(
		...
		'vtiger_bhscallcenterreport' => 'bhscallcenterreportid',
		'vtiger_bhscallcenterreportcf'=>'bhscallcenterreportid');
	...
	var $list_fields = Array (
        ...
        'Name' => Array('bhscallcenterreport', 'bhscallcenterreport_name'),
		'CreatedTime' => Array('crmentity', 'createdtime'),
	);
	var $list_fields_name = Array (
		/* Format: Field Label => fieldname */
		'Name' => 'bhscallcenterreport_name',
	...
	var $def_basicsearch_col = 'bhscallcenterreport_name';
	...
	var $mandatory_fields = Array('bhscallcenterreport_name');

	var $default_order_by = 'bhscallcenterreport_name';
	...
	function vtlib_handler($moduleName, $eventType) {
		...
		if($eventType == 'module.postinstall') {
			...
			vtws_deleteWebserviceEntity('BhsCallcenterReport');
    ...
    public function addDefaultModuleTypeEntity(){
		global $adb;
        // Check entity module
        $rs=$adb->query("SELECT * FROM `vtiger_ws_entity` WHERE `name`='BhsCallcenterReport'");
        if($adb->num_rows($rs) == 0) {
            ...
            $adb->pquery("INSERT INTO `vtiger_ws_entity` (`id`, `name`, `handler_path`, `handler_class`, `ismodule`) VALUES (?, ?, ?, ?, ?);",
                array($entityId, 'BhsCallcenterReport', 'include/Webservices/VtigerModuleOperation.php', 'VtigerModuleOperation', '1'));
        }
    }

    public function addEntityIdentifier(){
        global $adb;
        $moduleName = 'BhsCallcenterReport';
        ...
        if($adb->num_rows($result) > 0){
            ...
            if($adb->num_rows($result2)==0){
                $adb->pquery("INSERT INTO vtiger_entityname(tabid, modulename, tablename, fieldname, entityidfield, entityidcolumn) VALUES(?,?,?,?,?,?)",
                    Array($tabid, $moduleName, $this->table_name, 'bhscallcenterreport_name', $this->table_index, $this->table_index));
            }else{
