<?php
class BhsShippingPending extends CRMEntity {
	var $db, $log; // Used in class functions of CRMEntity

	var $table_name = 'vtiger_bhsshippingpending';
	var $table_index= 'bhsshippingpendingid';
	 ...
	 */
	var $customFieldTable = Array('vtiger_bhsshippingpendingcf', 'bhsshippingpendingid');

	/**
	 * Mandatory for Saving, Include tables related to this module.
	 */
	var $tab_name = Array('vtiger_crmentity', 'vtiger_bhsshippingpending', 'vtiger_bhsshippingpendingcf');
	...
	var $tab_name_index = Array(
		...
		'vtiger_bhsshippingpending' => 'bhsshippingpendingid',
		'vtiger_bhsshippingpendingcf'=>'bhsshippingpendingid');
	...
	var $list_fields = Array (
        ...
        'Name' => Array('bhsshippingpending', 'bhsshippingpending_name'),
		'CreatedTime' => Array('crmentity', 'createdtime'),
	);
	var $list_fields_name = Array (
		/* Format: Field Label => fieldname */
		'Name' => 'bhsshippingpending_name',
	...
	var $def_basicsearch_col = 'bhsshippingpending_name';
	...
	var $mandatory_fields = Array('bhsshippingpending_name');

	var $default_order_by = 'bhsshippingpending_name';
	...
	function vtlib_handler($moduleName, $eventType) {
		...
		if($eventType == 'module.postinstall') {
			...
			vtws_deleteWebserviceEntity('BhsShippingPending');
    ...
    public function addDefaultModuleTypeEntity(){
		global $adb;
        // Check entity module
        $rs=$adb->query("SELECT * FROM `vtiger_ws_entity` WHERE `name`='BhsShippingPending'");
        if($adb->num_rows($rs) == 0) {
            ...
            $adb->pquery("INSERT INTO `vtiger_ws_entity` (`id`, `name`, `handler_path`, `handler_class`, `ismodule`) VALUES (?, ?, ?, ?, ?);",
                array($entityId, 'BhsShippingPending', 'include/Webservices/VtigerModuleOperation.php', 'VtigerModuleOperation', '1'));
        }
    }

    public function addEntityIdentifier(){
        global $adb;
        $moduleName = 'BhsShippingPending';
        ...
        if($adb->num_rows($result) > 0){
            ...
            if($adb->num_rows($result2)==0){
                $adb->pquery("INSERT INTO vtiger_entityname(tabid, modulename, tablename, fieldname, entityidfield, entityidcolumn) VALUES(?,?,?,?,?,?)",
                    Array($tabid, $moduleName, $this->table_name, 'bhsshippingpending_name', $this->table_index, $this->table_index));
            }else{
