<?php
class BhsAgentGroup extends CRMEntity {
	var $db, $log; // Used in class functions of CRMEntity

	var $table_name = 'vtiger_bhsagentgroup';
	var $table_index= 'bhsagentgroupid';
	 ...
	 */
	var $customFieldTable = Array('vtiger_bhsagentgroupcf', 'bhsagentgroupid');

	/**
	 * Mandatory for Saving, Include tables related to this module.
	 */
	var $tab_name = Array('vtiger_crmentity', 'vtiger_bhsagentgroup', 'vtiger_bhsagentgroupcf');
	...
	var $tab_name_index = Array(
		...
		'vtiger_bhsagentgroup' => 'bhsagentgroupid',
		'vtiger_bhsagentgroupcf'=>'bhsagentgroupid');
	...
	var $list_fields = Array (
        ...
        'Name' => Array('bhsagentgroup', 'bhsagentgroup_name'),
		'CreatedTime' => Array('crmentity', 'createdtime'),
	);
	var $list_fields_name = Array (
		/* Format: Field Label => fieldname */
		'Name' => 'bhsagentgroup_name',
	...
	var $def_basicsearch_col = 'bhsagentgroup_name';
	...
	var $mandatory_fields = Array('bhsagentgroup_name');

	var $default_order_by = 'bhsagentgroup_name';
	...
	function vtlib_handler($moduleName, $eventType) {
		...
		if($eventType == 'module.postinstall') {
			...
			vtws_deleteWebserviceEntity('BhsAgentGroup');
    ...
    public function addDefaultModuleTypeEntity(){
		global $adb;
        // Check entity module
        $rs=$adb->query("SELECT * FROM `vtiger_ws_entity` WHERE `name`='BhsAgentGroup'");
        if($adb->num_rows($rs) == 0) {
            ...
            $adb->pquery("INSERT INTO `vtiger_ws_entity` (`id`, `name`, `handler_path`, `handler_class`, `ismodule`) VALUES (?, ?, ?, ?, ?);",
                array($entityId, 'BhsAgentGroup', 'include/Webservices/VtigerModuleOperation.php', 'VtigerModuleOperation', '1'));
        }
    }

    public function addEntityIdentifier(){
        global $adb;
        $moduleName = 'BhsAgentGroup';
        ...
        if($adb->num_rows($result) > 0){
            ...
            if($adb->num_rows($result2)==0){
                $adb->pquery("INSERT INTO vtiger_entityname(tabid, modulename, tablename, fieldname, entityidfield, entityidcolumn) VALUES(?,?,?,?,?,?)",
                    Array($tabid, $moduleName, $this->table_name, 'bhsagentgroup_name', $this->table_index, $this->table_index));
            }else{
