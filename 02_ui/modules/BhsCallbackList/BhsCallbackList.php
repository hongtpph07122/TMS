<?php
class BhsCallbackList extends CRMEntity {
	var $db, $log; // Used in class functions of CRMEntity

	var $table_name = 'vtiger_bhscallbacklist';
	var $table_index= 'bhscallbacklistid';
	 ...
	 */
	var $customFieldTable = Array('vtiger_bhscallbacklistcf', 'bhscallbacklistid');

	/**
	 * Mandatory for Saving, Include tables related to this module.
	 */
	var $tab_name = Array('vtiger_crmentity', 'vtiger_bhscallbacklist', 'vtiger_bhscallbacklistcf');
	...
	var $tab_name_index = Array(
		...
		'vtiger_bhscallbacklist' => 'bhscallbacklistid',
		'vtiger_bhscallbacklistcf'=>'bhscallbacklistid');
	...
	var $list_fields = Array (
        ...
        'Name' => Array('bhscallbacklist', 'bhscallbacklist_name'),
		'CreatedTime' => Array('crmentity', 'createdtime'),
	);
	var $list_fields_name = Array (
		/* Format: Field Label => fieldname */
		'Name' => 'bhscallbacklist_name',
	...
	var $def_basicsearch_col = 'bhscallbacklist_name';
	...
	var $mandatory_fields = Array('bhscallbacklist_name');

	var $default_order_by = 'bhscallbacklist_name';
	...
	function vtlib_handler($moduleName, $eventType) {
		...
		if($eventType == 'module.postinstall') {
			...
			vtws_deleteWebserviceEntity('BhsCallbackList');
    ...
    public function addDefaultModuleTypeEntity(){
		global $adb;
        // Check entity module
        $rs=$adb->query("SELECT * FROM `vtiger_ws_entity` WHERE `name`='BhsCallbackList'");
        if($adb->num_rows($rs) == 0) {
            ...
            $adb->pquery("INSERT INTO `vtiger_ws_entity` (`id`, `name`, `handler_path`, `handler_class`, `ismodule`) VALUES (?, ?, ?, ?, ?);",
                array($entityId, 'BhsCallbackList', 'include/Webservices/VtigerModuleOperation.php', 'VtigerModuleOperation', '1'));
        }
    }

    public function addEntityIdentifier(){
        global $adb;
        $moduleName = 'BhsCallbackList';
        ...
        if($adb->num_rows($result) > 0){
            ...
            if($adb->num_rows($result2)==0){
                $adb->pquery("INSERT INTO vtiger_entityname(tabid, modulename, tablename, fieldname, entityidfield, entityidcolumn) VALUES(?,?,?,?,?,?)",
                    Array($tabid, $moduleName, $this->table_name, 'bhscallbacklist_name', $this->table_index, $this->table_index));
            }else{
