<?php
class BhsOrders extends CRMEntity {
	var $db, $log; // Used in class functions of CRMEntity

	var $table_name = 'vtiger_bhsorders';
	var $table_index= 'bhsorderid';
	 ...
	 */
	var $customFieldTable = Array('vtiger_bhsorderscf', 'bhsorderid');

	/**
	 * Mandatory for Saving, Include tables related to this module.
	 */
	var $tab_name = Array('vtiger_crmentity', 'vtiger_bhsorders', 'vtiger_bhsorderscf');
	...
	var $tab_name_index = Array(
		...
		'vtiger_bhsorders' => 'bhsorderid',
		'vtiger_bhsorderscf'=>'bhsorderid');
	...
	var $list_fields = Array (
        ...
        'Name' => Array('bhsorders', 'bhsorder_name'),
	...
	var $def_basicsearch_col = 'bhsorder_name';
	...
	var $mandatory_fields = Array('bhsorder_name');

	var $default_order_by = 'bhsorder_name';
	...
	function vtlib_handler($moduleName, $eventType) {
		...
		if($eventType == 'module.postinstall') {
			...
			vtws_deleteWebserviceEntity('BhsOrders');
    ...
    public function addModuleIcon(){
        ...
        $source1 = $root_directory . '/modules/BhsOrders/resources/BhsOrders.png';
        $source2 = $root_directory . '/modules/BhsOrders/resources/moduleImages/BhsOrders.png';
    ...
    public function addDefaultModuleTypeEntity(){
		global $adb;
        // Check entity module
        $rs=$adb->query("SELECT * FROM `vtiger_ws_entity` WHERE `name`='BhsOrders'");
        if($adb->num_rows($rs) == 0) {
            ...
            $adb->pquery("INSERT INTO `vtiger_ws_entity` (`id`, `name`, `handler_path`, `handler_class`, `ismodule`) VALUES (?, ?, ?, ?, ?);",
                array($entityId, 'BhsOrders', 'include/Webservices/VtigerModuleOperation.php', 'VtigerModuleOperation', '1'));
        }

        // Add to menu
        $tabid = getTabid("BhsOrders");
        $appname = 'SALEORDER';
        ...
        if($adb->num_rows($result) > 0){
            ...
            if($adb->num_rows($result2)==0){
                $adb->pquery("INSERT INTO vtiger_entityname(tabid, modulename, tablename, fieldname, entityidfield, entityidcolumn) VALUES(?,?,?,?,?,?)",
                    Array($tabid, $moduleName, $this->table_name, 'bhsorder_name', $this->table_index, $this->table_index));
            }else{
