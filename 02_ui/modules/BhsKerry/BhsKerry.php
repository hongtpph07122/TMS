<?php
class BhsKerry extends CRMEntity {
	var $db, $log; // Used in class functions of CRMEntity

	var $table_name = 'vtiger_BhsKerry';
	var $table_index= 'BhsKerryid';
	 ...
	 */
	var $customFieldTable = Array('vtiger_BhsKerrycf', 'BhsKerryid');

	/**
	 * Mandatory for Saving, Include tables related to this module.
	 */
	var $tab_name = Array('vtiger_crmentity', 'vtiger_BhsKerry', 'vtiger_BhsKerrycf');
	...
	var $tab_name_index = Array(
		...
		'vtiger_BhsKerry' => 'BhsKerryid',
		'vtiger_BhsKerrycf'=>'BhsKerryid');
	...
	var $list_fields = Array (
        ...
        'Name' => Array('BhsKerry', 'BhsKerry_name'),
	...
	var $def_basicsearch_col = 'BhsKerry_name';
	...
	var $mandatory_fields = Array('BhsKerry_name');

	var $default_order_by = 'BhsKerry_name';
	...
	function vtlib_handler($moduleName, $eventType) {
		...
		if($eventType == 'module.postinstall') {
			...
			vtws_deleteWebserviceEntity('BhsKerry');
    ...
    public function addDefaultModuleTypeEntity(){
		global $adb;
        // Check entity module
        $rs=$adb->query("SELECT * FROM `vtiger_ws_entity` WHERE `name`='BhsKerry'");
        if($adb->num_rows($rs) == 0) {
            ...
            $adb->pquery("INSERT INTO `vtiger_ws_entity` (`id`, `name`, `handler_path`, `handler_class`, `ismodule`) VALUES (?, ?, ?, ?, ?);",
                array($entityId, 'BhsKerry', 'include/Webservices/VtigerModuleOperation.php', 'VtigerModuleOperation', '1'));
        }
    }
        ...
        if($adb->num_rows($result) > 0){
            ...
            if($adb->num_rows($result2)==0){
                $adb->pquery("INSERT INTO vtiger_entityname(tabid, modulename, tablename, fieldname, entityidfield, entityidcolumn) VALUES(?,?,?,?,?,?)",
                    Array($tabid, $moduleName, $this->table_name, 'BhsKerry_name', $this->table_index, $this->table_index));
            }else{
