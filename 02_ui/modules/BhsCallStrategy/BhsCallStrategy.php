<?php
class BhsCallStrategy extends CRMEntity {
	var $db, $log; // Used in class functions of CRMEntity

	var $table_name = 'vtiger_bhscallstrategy';
	var $table_index= 'bhscallstrategyid';
	 ...
	 */
	var $customFieldTable = Array('vtiger_bhscallstrategycf', 'bhscallstrategyid');

	/**
	 * Mandatory for Saving, Include tables related to this module.
	 */
	var $tab_name = Array('vtiger_crmentity', 'vtiger_bhscallstrategy', 'vtiger_bhscallstrategycf');
	...
	var $tab_name_index = Array(
		...
		'vtiger_bhscallstrategy' => 'bhscallstrategyid',
		'vtiger_bhscallstrategycf'=>'bhscallstrategyid');
	...
	var $list_fields = Array (
        ...
        'Name' => Array('bhscallstrategy', 'bhscallstrategy_name'),
		'CreatedTime' => Array('crmentity', 'createdtime'),
	);
	var $list_fields_name = Array (
		/* Format: Field Label => fieldname */
		'Name' => 'bhscallstrategy_name',
	...
	var $def_basicsearch_col = 'bhscallstrategy_name';
	...
	var $mandatory_fields = Array('bhscallstrategy_name');

	var $default_order_by = 'bhscallstrategy_name';
	...
	function vtlib_handler($moduleName, $eventType) {
		...
		if($eventType == 'module.postinstall') {
			...
			vtws_deleteWebserviceEntity('bhscallstrategy');
    ...
    public function addModuleIcon(){
        ...
        $source1 = $root_directory . '/modules/BhsCallStrategy/resources/BhsCallStrategy.png';
        $source2 = $root_directory . '/modules/BhsCallStrategy/resources/moduleImages/BhsOrders.png';
        $dest1 = $root_directory . '/layouts/v7/skins/images/BhsOrders.png';
        copy($source1, $dest1);
        $dest2 = $root_directory . '/layouts/v7/skins/images/moduleImages/BhsOrders.png';
        copy($source2, $dest2);
    }
    ...
    public function addDefaultModuleTypeEntity(){
		global $adb;
        // Check entity module
        $rs=$adb->query("SELECT * FROM `vtiger_ws_entity` WHERE `name`='BhsCallStrategy'");
        if($adb->num_rows($rs) == 0) {
            ...
            $adb->pquery("INSERT INTO `vtiger_ws_entity` (`id`, `name`, `handler_path`, `handler_class`, `ismodule`) VALUES (?, ?, ?, ?, ?);",
                array($entityId, 'BhsCallStrategy', 'include/Webservices/VtigerModuleOperation.php', 'VtigerModuleOperation', '1'));
        }

        // Add to menu
        $tabid = getTabid("BhsCallStrategy");
        $appname = 'BHS_CAMPAIGN';
        ...
        if($adb->num_rows($result) > 0){
            ...
            if($adb->num_rows($result2)==0){
                $adb->pquery("INSERT INTO vtiger_entityname(tabid, modulename, tablename, fieldname, entityidfield, entityidcolumn) VALUES(?,?,?,?,?,?)",
                    Array($tabid, $moduleName, $this->table_name, 'bhssearchorders_name', $this->table_index, $this->table_index));
            }else{
