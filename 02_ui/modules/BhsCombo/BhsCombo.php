<?php
class BhsCombo extends CRMEntity {
	var $db, $log; // Used in class functions of CRMEntity

	var $table_name = 'vtiger_BhsCombo';
	var $table_index= 'BhsComboid';
	 ...
	 */
	var $customFieldTable = Array('vtiger_BhsCombocf', 'BhsComboid');

	/**
	 * Mandatory for Saving, Include tables related to this module.
	 */
	var $tab_name = Array('vtiger_crmentity', 'vtiger_BhsCombo', 'vtiger_BhsCombocf');
	...
	var $tab_name_index = Array(
		...
		'vtiger_BhsCombo' => 'BhsComboid',
		'vtiger_BhsCombocf'=>'BhsComboid');
	...
	var $list_fields = Array (
        ...
        'Name' => Array('BhsCombo', 'BhsCombo_name'),
	...
	var $def_basicsearch_col = 'BhsCombo_name';
	...
	var $mandatory_fields = Array('BhsCombo_name');

	var $default_order_by = 'BhsCombo_name';
	...
	function vtlib_handler($moduleName, $eventType) {
		...
		if($eventType == 'module.postinstall') {
			...
			vtws_deleteWebserviceEntity('BhsCombo');
    ...
    public function addDefaultModuleTypeEntity(){
		global $adb;
        // Check entity module
        $rs=$adb->query("SELECT * FROM `vtiger_ws_entity` WHERE `name`='BhsCombo'");
        if($adb->num_rows($rs) == 0) {
            ...
            $adb->pquery("INSERT INTO `vtiger_ws_entity` (`id`, `name`, `handler_path`, `handler_class`, `ismodule`) VALUES (?, ?, ?, ?, ?);",
                array($entityId, 'BhsCombo', 'include/Webservices/VtigerModuleOperation.php', 'VtigerModuleOperation', '1'));
        }
    }
        ...
        if($adb->num_rows($result) > 0){
            ...
            if($adb->num_rows($result2)==0){
                $adb->pquery("INSERT INTO vtiger_entityname(tabid, modulename, tablename, fieldname, entityidfield, entityidcolumn) VALUES(?,?,?,?,?,?)",
                    Array($tabid, $moduleName, $this->table_name, 'BhsCombo_name', $this->table_index, $this->table_index));
            }else{
